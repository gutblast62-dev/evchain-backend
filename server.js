...
app.post('/api/auth/register', authLimiter, (req, res) => {
  ...
  db.prepare(`INSERT INTO users (id, username, password_hash, name, badge, role, is_active) VALUES (?, ?, ?, ?, ?, ?, ?)`)
    .run(id, username.toLowerCase(), bcrypt.hashSync(password, 12), name, badge || null, 'personnel', 0);
...
app.get('/api/evidence/:id/timeline', authenticateToken, (req, res) => {
  ...
  const custodyEvents = db.prepare('SELECT ce.*, "custody" as event_type FROM custody_events ce WHERE ce.evidence_id = ? ORDER BY ce.date ASC').all(req.params.id);
  const auditEvents = req.user.role === 'admin'
    ? db.prepare('SELECT al.*, "audit" as event_type FROM audit_log al WHERE al.evidence_id = ? ORDER BY al.created_at ASC').all(req.params.id)
    : [];
  const timeline = [...custodyEvents, ...auditEvents].sort((a, b) => new Date(a.date || a.created_at) - new Date(b.date || b.created_at));
  res.json(timeline);
});
