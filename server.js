...
app.use(helmet());
app.use(cors({
  origin: process.env.FRONTEND_URL || 'https://gutblast62-dev.github.io',
  credentials: true
}));
...
// Get all evidence (with access control)
app.get('/api/evidence', authenticateToken, (req, res) => {
  ...
  // Personnel can only see their assigned evidence
...
// Create evidence (admin only)
app.post('/api/evidence', authenticateToken, requireAdmin, (req, res) => {
  const {
    caseNumber, description, type, status,
    assignedOfficer, collectedBy, dateCollected,
    location, notes
  } = req.body;
  ...
  db.prepare(`
    INSERT INTO evidence (id, case_number, description, type, status, assigned_officer, 
                          collected_by, date_collected, location, notes)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(id, caseNumber, description, type, status || 'Collected', assignedOfficer,
         collectedBy || assignedOfficer, dateCollected, location || null, notes || null);
...
app.delete('/api/evidence/:id', authenticateToken, requireAdmin, (req, res) => {
  ...
  try {
    ...
    if (err.message.includes('ON DELETE RESTRICT')) {
      return res.status(409).json({ error: 'Cannot delete evidence with custody events. Remove custody records first.' });
    }
    throw err;
  }
});
...
app.get('/api/evidence/:id/custody', authenticateToken, (req, res) => {
  ...
  const events = db.prepare(`
    SELECT * FROM custody_events WHERE evidence_id = ? ORDER BY date ASC
  `).all(id);

  res.json(events);
});

// Add custody event (admin only)
app.post('/api/evidence/:id/custody', authenticateToken, requireAdmin, (req, res) => {
  ...
  // Update evidence status if provided
  if (newStatus) {
...
// Delete user (admin only, can't delete admin)
...
app.post('/api/evidence/:id/qr', authenticateToken, (req, res) => {
  ...
  db.prepare(`
    INSERT INTO qr_tokens (id, evidence_id, token)
    VALUES (?, ?, ?)
...
app.get('/api/qr/:token', (req, res) => {
  ...
  if (!qr) {
    logAudit('QR_ACCESS_DENIED', null, null, `Invalid token: ${token}`, req.ip);
    return res.status(404).json({ error: 'Invalid or expired QR code' });
  }

  // Update scan count
  db.prepare(`
    UPDATE qr_tokens SET scan_count = scan_count + 1, last_scanned = CURRENT_TIMESTAMP WHERE id = ?
  `).run(qr.id);

  logAudit('QR_SCANNED', null, qr.evidence_id, `Token: ${token}`, req.ip);
...
app.get('/api/stats', authenticateToken, (req, res) => {
  ...
  if (req.user.role === 'personnel') {
    ...
    custodyQuery = `SELECT COUNT(*) as events FROM custody_events ce 
                    JOIN evidence e ON ce.evidence_id = e.id 
                    WHERE e.assigned_officer = ?`;
    params.push(req.user.username);
  }
...
app.listen(PORT, () => {
  console.log(`🚀 EVCHAIN API Server running on port ${PORT}`);
