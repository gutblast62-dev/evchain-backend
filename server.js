...
app.post('/api/evidence', authenticateToken, requireAdmin, (req, res) => {
  ...
  if (!caseNumber || !description || !type || !assignedOfficer || !dateCollected) return res.status(400).json({ error: 'Missing required fields' });
...
app.delete('/api/evidence/:id', authenticateToken, requireAdmin, (req, res) => {
  ...
  try {
    ...
    if (err.message.includes('ON DELETE RESTRICT')) return res.status(409).json({ error: 'Cannot delete evidence with custody events' });
    throw err;
  }
});
...
app.post('/api/evidence/:id/custody', authenticateToken, requireAdmin, (req, res) => {
  ...
  db.prepare(`INSERT INTO custody_events (evidence_id, case_number, action, handler, date, notes, new_status)
    VALUES (?, ?, ?, ?, ?, ?, ?)`).run(id, ev.case_number, action, handler, date, notes || null, newStatus || null);

  if (newStatus) db.prepare('UPDATE evidence SET status = ? WHERE id = ?').run(newStatus, id);
...
app.post('/api/users', authenticateToken, requireAdmin, (req, res) => {
  ...
  db.prepare(`INSERT INTO users (id, username, password_hash, name, badge, role)
    VALUES (?, ?, ?, ?, ?, ?)`).run(id, username.toLowerCase(), passwordHash, name, badge || null, role || 'personnel');
...
app.get('/api/qr/:token', (req, res) => {
  ...
  const qr = db.prepare(`SELECT qr_tokens.*, evidence.case_number, evidence.description 
    FROM qr_tokens JOIN evidence ON qr_tokens.evidence_id = evidence.id
    WHERE qr_tokens.token = ? AND qr_tokens.is_revoked = 0
    AND (qr_tokens.expires_at IS NULL OR qr_tokens.expires_at > CURRENT_TIMESTAMP)`).get(token);
  if (!qr) {
