...
// ============================================
// CORS - FIXED FOR RAILWAY + GITHUB PAGES
// ============================================
// Must be FIRST middleware - before ANY other app.use()
const allowedOrigins = [
  'https://gutblast62-dev.github.io',
  'https://gutblast62-dev.github.io/evchain-frontend',
  'http://localhost:3000',
  'http://localhost:5173',
  'http://localhost:8080',
  'http://127.0.0.1:3000',
...
// Handle OPTIONS preflight for ALL routes - MUST come before routes
app.options('*', cors());
...
// Rate limiting - FIXED for Railway (trust proxy)
...
// ============================================
// HEALTH CHECK - FIXED (must return JSON for API)
// ============================================
app.get('/', (req, res) => {
  res.json({ 
    status: 'EVCHAIN API running', 
...
app.get('/health', (req, res) => {
  res.json({ status: 'ok', db: 'connected', cors: 'enabled', timestamp: new Date().toISOString() });
});
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`🌐 CORS: enabled for GitHub Pages + localhost`);
});
