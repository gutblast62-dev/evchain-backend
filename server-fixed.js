...
// Add more origins to EXTRA_ORIGINS in your .env (comma-separated):
//   EXTRA_ORIGINS=https://myotherfrontend.com,http://192.168.1.10:8080
const BUILT_IN_ORIGINS = [
  'https://gutblast62-dev.github.io',   // GitHub Pages root
  'http://localhost:3000',
  'http://localhost:5500',
  'http://localhost:8080',
  'http://127.0.0.1:3000',
...
function getAllowedOrigins() {
  const origins = [...BUILT_IN_ORIGINS];
  // Add FRONTEND_URL from .env
  if (process.env.FRONTEND_URL) {
    // Strip trailing slash and /Evchain path — we match on the origin (scheme+host only)
    const base = process.env.FRONTEND_URL.replace(/\/$/, '').replace(/\/[^/]+$/, (m) =>
      m.includes('.') ? m : '' // keep if it looks like a TLD, strip if it's a path
    );
    if (base && !origins.includes(base)) origins.push(base);
    if (!origins.includes(process.env.FRONTEND_URL)) origins.push(process.env.FRONTEND_URL);
  }
  // Add any extra origins from .env
  if (process.env.EXTRA_ORIGINS) {
    process.env.EXTRA_ORIGINS.split(',').map(o => o.trim()).filter(Boolean).forEach(o => {
...
const corsOptions = {
  origin: function (origin, callback) {
    ...
    // GitHub Pages: allow any path under gutblast62-dev.github.io
    if (origin.startsWith('https://gutblast62-dev.github.io')) return callback(null, true);
    ...
    console.warn(`[CORS] Allowed origins: ${allowed.join(', ')}`);
...
// Apply CORS FIRST — before every other middleware
app.use(cors(corsOptions));

// Explicitly handle preflight OPTIONS for ALL routes
app.options('*', cors(corsOptions));

// Belt-and-suspenders: manually set CORS headers in case the cors() middleware
// is bypassed (e.g. by a reverse proxy that strips headers)
app.use((req, res, next) => {
  const origin = req.headers.origin;
  if (origin) {
    const allowed = getAllowedOrigins();
    const isAllowed = allowed.includes(origin)
      || origin.startsWith('https://gutblast62-dev.github.io')
